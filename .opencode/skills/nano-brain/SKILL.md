...
Installation, environment, and platform escape hatches.

### Install nano-brain globally without `sudo`

```bash
npm install -g --prefix ~/.local @nano-step/nano-brain@latest
```

The binary lands at `~/.local/bin/nano-brain`. Ensure `~/.local/bin` is on your `$PATH` (most Linux distros include it by default; macOS does not).

### Skip SHA-256 verification (corp proxy / air-gapped installs)

`npm install @nano-step/nano-brain` verifies the downloaded Go binary against the release `SHA256SUMS` asset. If a corporate proxy modifies the download stream or you're installing in an air-gapped environment, set:

```bash
NANO_BRAIN_SKIP_SHA_VERIFY=1 npm install -g @nano-step/nano-brain@latest
```

**Security trade-off:** this bypasses the supply-chain integrity check. A WARN line is printed so the bypass is visible in CI logs. Only use when you control the network path. See issue [#320](https://github.com/nano-step/nano-brain/issues/320).

### macOS Gatekeeper quarantine

If macOS refuses to run the downloaded binary (`"nano-brain" cannot be opened because the developer cannot be verified`), clear the quarantine attribute:

```bash
xattr -dr com.apple.quarantine "$(which nano-brain)"
```

Applies to any binary downloaded outside the App Store. Re-run after each `npm install` if Gatekeeper re-flags the file.

### Override the MCP URL (`NANO_BRAIN_MCP_URL`)

The nano-brain skill resolves the MCP URL with this precedence:

1. `NANO_BRAIN_MCP_URL` environment variable (if set and non-empty)
2. `/.dockerenv` exists → `http://host.docker.internal:3100/mcp`
3. Default → `http://localhost:3100/mcp`

Set it explicitly when running on a non-default port, a remote VPS, or a container runtime that doesn't expose `/.dockerenv`:

```bash
# Container talking to host nano-brain
export NANO_BRAIN_MCP_URL=http://host.docker.internal:3100/mcp

# Bare-metal host
export NANO_BRAIN_MCP_URL=http://localhost:3100/mcp

# Remote VPS deployment
export NANO_BRAIN_MCP_URL=https://nano-brain.internal.example.com/mcp
```

The OpenCode skill manifest reads this env var via `{env:NANO_BRAIN_MCP_URL}` substitution (OpenCode's native config templating). Setting the env var in your shell rc / launchd / systemd unit propagates to every nano-brain MCP call.

### Override the binary path (`NANO_BRAIN_BIN`)

When set, the npm shim execs the path from `NANO_BRAIN_BIN` instead of the npm-installed binary. The path must exist and be executable:

```bash
export NANO_BRAIN_BIN=/usr/local/bin/nano-brain-custom
```

**Failure modes** (fail loudly, no silent fallback):

| Condition | Result |
|---|---|
| File does not exist | Shim exits 1 with `Error: NANO_BRAIN_BIN points to /path which does not exist.` |
| File exists but `mode & 0111 == 0` | Shim exits 1 with `Error: NANO_BRAIN_BIN points to /path which is not executable. Run: chmod +x /path` |
| Env var unset or empty | Falls through to default resolution (npm bin or PATH lookup) |

Use this to point at a self-built binary (`go build -o ~/bin/nano-brain ./cmd/nano-brain`) while leaving the npm install intact.
