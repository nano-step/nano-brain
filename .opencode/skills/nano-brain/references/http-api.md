...
description: Direct HTTP integration patterns for nano-brain — for scripts, dashboards, custom tooling, and tests that bypass the MCP layer.
---
...
Watch `queue_pending` over time — if it grows without bound and `queue_status == "backpressure"`, embedding throughput is below ingestion rate. Check ollama health via `GET /api/status` (look at `active_provider` field) or probe the embedding provider URL directly.
...
2. **Acknowledge the risk** with `--unsafe-no-auth` daemon startup flag (e.g. inside a private network or VPN).
