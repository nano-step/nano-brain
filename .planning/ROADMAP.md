...
- [ ] **Phase 17: Agent-ergonomics fixes (issue #539)** - Field report from an autonomous agent: (F1) `memory_trace` nodes are bare identifiers not `file::symbol`, polluted with language builtins/third-party, and collision-prone → resolve targets to workspace symbols at query time, filter externals, distinguish same-named methods; (F2) `memory_get "#<uuid>"` only resolves document_id and leaks a raw SQL error on a chunk id → fall back to chunk→parent-document + clean error; (F3) symbols persist only the signature and no line span → add EndLine to extractors, persist start/end line, emit them from `memory_symbols`, return full body from `memory_get` on a symbol path; (F4) hybrid `memory_query` 18–58s vs 82ms BM25 → in-process query-embedding cache + embed-client review. Deferred: extraction-time scope-aware call resolution (#501), pool/batch tuning (#372/#373). See `.planning/phases/17-agent-ergonomics-issue-539/DESIGN.md`.
...
- [ ] **Phase 18: Agent-supplied query expansion, provider-free HyDE (issue #588, #543 D2)** - `memory_query` gains an optional `hypothetical` string: when the calling agent supplies its own ideal-answer paragraph, the vector leg embeds it directly and server-side HyDE (`s.hydeGenerator.Generate`) is skipped entirely — the agent's own model replaces a separately-configured server-side LLM. Additive only: omitting `hypothetical` is byte-identical to today (existing config-driven HyDE unchanged, BM25 leg always uses the raw query). Plumbed through `HybridSearch`'s new trailing param, the `memory_query` MCP tool, and REST `/api/v1/query` for parity. MCP sampling (server→client `createMessage`) was considered and rejected for now — blocked by the stateless streamable-HTTP transport and unverified client support; deferred to roadmap below. See `.planning/phases/18-search-hot-path-stage-deadline/DESIGN.md`.

- [ ] **Roadmap note**: MCP sampling for provider-free HyDE (needs non-stateless transport + client support; SDK TODO #148).
