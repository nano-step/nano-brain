...
// RunTask calls every tool listed in the task, unions the surfaced strings, then scores recall.
func RunTask(ctx context.Context, client *http.Client, cfg Config, task Task) TaskResult {
	var names []string
	var paths []string
	...
	return scoreTask(task, names, paths)
}
