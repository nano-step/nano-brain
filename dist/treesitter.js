...
export async function parseToAST(code, language) {
    await initPromise;
...
function extractObjectProperties(objectNode, parentName, filePath, symbols) {
...
function extractTsJsSymbols(rootNode, filePath) {
    const symbols = [];
...
function extractPythonSymbols(rootNode, filePath) {
    const symbols = [];
    walkTree(rootNode, (node) => {
...
export async function parseSymbols(filePath, content, language) {
    await initPromise;
    if (!treeSitterAvailable || !Parser) {
        return [];
    }
    ...
    try {
        ...
        if (parseSymbols._dbgCount === undefined)
            parseSymbols._dbgCount = 0;
        if (parseSymbols._dbgCount++ < 3) {
...
function findEnclosingSymbol(node, language) {
    let current = node.parent;
    while (current) {
...
function extractCallExpressions(rootNode, language) {
    const calls = [];
    walkTree(rootNode, (node) => {
...
function findQualifiedTarget(callName, isMemberExpression, symbolTable, sourceFilePath) {
...
export async function resolveCallEdges(filePath, content, language, symbolTable) {
    await initPromise;
    if (!treeSitterAvailable || !Parser) {
        return [];
    }
...
function extractHeritageInfo(rootNode, language) {
    const heritage = [];
    walkTree(rootNode, (node) => {
        if (language === 'ts' || language === 'js') {
            if (node.type === 'class_declaration') {
...
export async function resolveHeritageEdges(filePath, content, language, symbolTable) {
    await initPromise;
    if (!treeSitterAvailable || !Parser) {
        return [];
    }
