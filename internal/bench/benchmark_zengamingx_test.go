...
type httpSearcherZengamingx struct {
	baseURL    string
	workspace  string
	httpClient *http.Client
}

func (h *httpSearcherZengamingx) HybridSearch(ctx context.Context, query string, workspace string, maxResults int, tags []string, timeRange *search.TimeRangeFilter, chunkType string) ([]search.Result, error) {
	body := map[string]any{
		"query":       query,
		"workspace":   workspace,
...
func loadBaselineZengamingx(t *testing.T) *BenchmarkResults {
	t.Helper()
	data, err := os.ReadFile("testdata/baseline_zengamingx_v1.json")
	if err != nil {
...
func saveResultsZengamingx(t *testing.T, results *BenchmarkResults) {
	t.Helper()
	data, err := json.MarshalIndent(results, "", "  ")
	if err != nil {
		t.Fatalf("failed to marshal results: %v", err)
	}
	if err := os.WriteFile("testdata/results_zengamingx_current.json", data, 0644); err != nil {
		t.Fatalf("failed to write results: %v", err)
	}
}

func TestBenchmarkZengamingx(t *testing.T) {
	searcher := &httpSearcherZengamingx{
	...
	saveResultsZengamingx(t, results)

	fmt.Println("=== Zengamingx Search Benchmark ===")
	...
	baseline := loadBaselineZengamingx(t)
