...
func Reload(configPath string, current *Config) (*Config, *ReloadResult, error) {
	...
	fields := []field{
		...
		{"embedding.concurrency", false, newCfg.Embedding.Concurrency != current.Embedding.Concurrency},
		...
		{"storage.max_file_size", false, newCfg.Storage.MaxFileSize != current.Storage.MaxFileSize},
		{"storage.max_size", false, newCfg.Storage.MaxSize != current.Storage.MaxSize},
		{"watcher.debounce_ms", false, newCfg.Watcher.DebounceMs != current.Watcher.DebounceMs},
		{"watcher.reindex_interval", false, newCfg.Watcher.ReindexInterval != current.Watcher.ReindexInterval},
		{"intervals.session_poll", false, newCfg.Intervals.SessionPoll != current.Intervals.SessionPoll},
