...
func extractTSDeclarationIdentities(bt *gotreesitter.BoundTree, tree *gotreesitter.Tree, content []byte, filePath string, lang *gotreesitter.Language) []Edge {
	var edges []Edge
	add := func(node *gotreesitter.Node, target string) {
		edges = append(edges, Edge{SourceNode: filePath, TargetNode: target, Kind: EdgeContains, SourceFile: filePath, Line: lineForByte(content, node.StartByte()), Language: "typescript"})
	}
