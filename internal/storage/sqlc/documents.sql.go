...
const deleteLegacySummaryDocsForSymbol = `-- name: DeleteLegacySummaryDocsForSymbol :execrows
DELETE FROM documents
WHERE workspace_hash = $1
  AND starts_with(source_path, $2)
`

type DeleteLegacySummaryDocsForSymbolParams struct {
	WorkspaceHash string
	PathPrefix    string
}
...
func (q *Queries) DeleteLegacySummaryDocsForSymbol(ctx context.Context, arg DeleteLegacySummaryDocsForSymbolParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, deleteLegacySummaryDocsForSymbol, arg.WorkspaceHash, arg.PathPrefix)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
const resolveSymbolByName = `-- name: ResolveSymbolByName :many
...
type ResolveSymbolByNameParams struct {
	WorkspaceHash string
	Column2       string
}

type ResolveSymbolByNameRow struct {
	SourcePath string
	Metadata   pqtype.NullRawMessage
}
...
func (q *Queries) ResolveSymbolByName(ctx context.Context, arg ResolveSymbolByNameParams) ([]ResolveSymbolByNameRow, error) {
	rows, err := q.db.QueryContext(ctx, resolveSymbolByName, arg.WorkspaceHash, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ResolveSymbolByNameRow
	for rows.Next() {
		var i ResolveSymbolByNameRow
		if err := rows.Scan(&i.SourcePath, &i.Metadata); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
