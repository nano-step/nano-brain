...
const listAllEdgesByWorkspace = `-- name: ListAllEdgesByWorkspace :many
SELECT id, workspace_hash, source_node, target_node, edge_type, source_file, metadata, created_at
FROM graph_edges
WHERE workspace_hash = $1
...
func (q *Queries) ListAllEdgesByWorkspace(ctx context.Context, workspaceHash string) ([]GraphEdge, error) {
	rows, err := q.db.QueryContext(ctx, listAllEdgesByWorkspace, workspaceHash)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listConsumerEntryNodesByWorkspace = `-- name: ListConsumerEntryNodesByWorkspace :many
SELECT id, workspace_hash, source_node, target_node, edge_type, source_file, metadata, created_at
FROM graph_edges
WHERE workspace_hash = $1
...
func (q *Queries) ListConsumerEntryNodesByWorkspace(ctx context.Context, workspaceHash string) ([]GraphEdge, error) {
	rows, err := q.db.QueryContext(ctx, listConsumerEntryNodesByWorkspace, workspaceHash)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
