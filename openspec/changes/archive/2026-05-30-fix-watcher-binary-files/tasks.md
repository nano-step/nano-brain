...
- [x] **A1** — Confirm existing watcher tests pass on `b-main`:
  ```bash
...
- [x] **B1** — Create new file `internal/watcher/binary.go` with:
...
- [x] **B2** — Create `internal/watcher/binary_test.go` covering:
...
- [x] **B3** — `go test -race -short ./internal/watcher/...` PASS. Commit: `feat(watcher): add binary file detection helpers (#252)`

## Phase C — Wire into processFile (closes the actual bug)

- [x] **C1** — Modify `internal/watcher/watcher.go` `processFile()` (around line 341-355):
...
- [x] **C2** — Verify `go build ./...` passes (no import issues).

- [x] **C3** — Run watcher unit tests: `go test -race -short ./internal/watcher/...` — must remain green (no regression).

- [x] **C4** — Commit: `fix(watcher): skip binary files before UTF-8 upsert (#252)`
...
- [x] **D1** — Append to `internal/watcher/watcher_test.go`:
...
- [x] **D2** — `go test -race -short ./internal/watcher/... -v` → all 3 new tests PASS + no regression. Commit: `test(watcher): cover binary file skip + valid UTF-8 happy path (#252)`

## Phase E — Validation ladder

- [x] **E1** — `validate:quick`: `go build ./... && go test -race -short ./...` → green. Paste output to story Evidence.

- [x] **E2** — `test:integration`: `go test -race -tags=integration ./...` → green for all packages I touched. (Pre-existing `internal/search/isolation_test.go` build break from #221 remains out of scope.)

- [x] **E3** — `smoke:e2e` (per bug-fix change-type, see HARNESS validation ladder):
  - Build binary
...
- [x] **E4** — `self-review:staged-files`: `git status` clean before each commit (no `.opencode/`, no `node_modules/`).

## Phase F — PR + review gate

- [x] **F1** — Push branch: `git push -u origin fix/252-watcher-binary-files`

- [x] **F2** — Open PR with title `fix(watcher): skip binary files (#252)` and body containing `Closes #252`. Reference OpenSpec change folder + evidence.

- [x] **F3** — Wait for Gemini PR bot review (org-level GitHub App, auto-fires).

- [x] **F4** — Triage Gemini comments per R31 rule in `docs/HARNESS.md`. Record in `docs/evidence/fix-watcher-binary-files/gemini-triage.md`.

- [x] **F5** — On Gemini PASS + Review Gate PASS: merge via squash. Bot review loop max 3 cycles.

- [x] **F6** — Tag + npm publish (follow existing pattern from #238 / v2026.5.3006):
  ```bash
...
- [x] **F7** — `openspec archive fix-watcher-binary-files --yes` to finalize specs.
