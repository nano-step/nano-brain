...
- [x] **A1** Add `isHardFailureEmbedError(err error) bool` helper to `internal/embed/queue.go`. Returns true for substrings matching:
...
- [x] **A2** In `processChunk()`, after `q.embedder.Embed()` error branch, classify before retry:
  ```go
  if err != nil {
...
- [x] **A3** Run `go build ./...` and `go vet ./...` — both clean.

## Phase B — Tests

- [x] **B1** `TestIsHardFailureEmbedError` — table-driven, 10+ cases covering all 5 status codes (positive), 3 transient codes (500/502/503 → false), bare errors (false).

- [x] **B2** `TestProcessChunk_HardFailOn400` — use mockEmbedder returning `fmt.Errorf("ollama: unexpected status 400: %s", "...")`. Assert:
...
- [x] **B3** `TestProcessChunk_TransientErrorRetries` — mock returns `fmt.Errorf("connection refused")`. Assert:
  - `increaseBackoff` called (verify `q.backoff` > 0)
  - `MarkChunkEmbedFailed` NOT called
  - retry map populated

- [x] **B4** Run `go test -race -short ./internal/embed/... -v` → all PASS including 3 new tests.

## Phase C — Validate ladder

- [x] **C1** `validate:quick`: `go build ./... && go test -race -short ./...` → green.

- [x] **C2** `self-review:staged-files`: `git status` clean before commit.
...
- [x] **D2** Push, open PR with `Closes #260`, await Gemini, triage per R31 if needed.

- [x] **D3** Squash merge.

- [x] **D4** Close issue, archive openspec, cleanup worktree + branch.
