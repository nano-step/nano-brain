...
- [ ] **A1** Verify baseline tests green:
  ```bash
  go test -race -short ./internal/summarize/... ./internal/config/...
  ```

- [ ] **A2** Read existing code surface:
...
- [ ] **B1** Create `internal/summarize/slug.go`:
...
- [ ] **B2** Create `internal/summarize/slug_test.go`:
...
- [ ] **C2** Create `internal/summarize/diskpath_test.go`:
...
- [ ] **C3** Run: `go test -race -short ./internal/summarize/...`
  Commit: `feat(summary): add disk path generation helper (#258)`

## Phase D — Atomic write helper

- [ ] **D1** Create `internal/summarize/diskwrite.go`:
...
- [ ] **D2** Create `internal/summarize/diskwrite_test.go`:
...
- [ ] **E2** Modify config Load: tilde-expand `OutputDir` at load time. Test in `internal/config/config_test.go`.

- [ ] **E3** Remove the existing `TestSummarizationConfig_OutputDirIgnored` test (or invert it to `TestSummarizationConfig_OutputDirHonored`).

- [ ] **E4** Update README and `examples/config.yml` to document `write_to_disk` default and `output_dir`.

- [ ] **E5** Run: `go test -race -short ./internal/config/...`
  Commit: `feat(config): add write_to_disk flag and tilde-expand output_dir (#258)`

## Phase F — Wire into Persister

- [ ] **F1** Modify `internal/summarize/persist.go`:
...
- [ ] **F2** Update `cmd/nano-brain/main.go` `buildHarvestSummarizer()`:
  - Read `cfg.Summarization.WriteToDisk` (default true if nil)
  - Read `cfg.Summarization.OutputDir`
  - Pass to `NewPersister`

- [ ] **F3** Add integration test `internal/summarize/persist_disk_test.go`:
...
- [ ] **F4** Run: `go test -race ./internal/summarize/... ./cmd/nano-brain/...`
  Commit: `feat(summary): wire disk persistence into Persister (#258)`

## Phase G — Backfill CLI

- [ ] **G1** Create `cmd/nano-brain/cmd_backfill_summaries.go`:
...
- [ ] **G2** Register command in `cmd/nano-brain/main.go` switch dispatcher.

- [ ] **G3** Create `cmd/nano-brain/cmd_backfill_summaries_test.go`:
...
- [ ] **G4** Update README CLI table with new command.

- [ ] **G5** Run: `go test -race ./cmd/nano-brain/...`
  Commit: `feat(cli): add backfill-summaries command for one-shot export (#258)`

## Phase H — End-to-end verification on live server

- [ ] **H1** On live `host.docker.internal:3100` server (operator must stop + rebuild + restart):
  - Build binary
  - Stop server
...
- [ ] **H2** Backfill existing 167 summaries:
  ```bash
...
- [ ] **H3** Verify file content matches DB content:
  - Pick any file
  - Get same doc via API `/api/v1/get`
  - `diff <(cat file.md) <(curl ... | jq -r .content)` → should be empty

- [ ] **H4** Open one file in Obsidian (or `cat`) — confirm readable markdown.

- [ ] **H5** Document evidence at `docs/evidence/restore-summary-disk-persistence/`:
...
- [ ] **I2** `test:integration`: `go test -race -tags=integration $(go list ./... | grep -v internal/search)` → green.
- [ ] **I3** `smoke:e2e`: build binary, start server with default config, harvest 1 session, verify file appears.
- [ ] **I4** `self-review:staged-files`: `git status` clean before each commit.

## Phase J — Release notes

- [ ] **J1** CHANGELOG.md entry under Unreleased:
...
- [ ] **K1** Push branch.
- [ ] **K2** Open PR with title `feat(summary): restore disk persistence for Obsidian compatibility (#258)`. Body links OpenSpec proposal + evidence.
- [ ] **K3** Wait for Gemini review. Triage per R31 in `docs/evidence/.../gemini-triage.md`.
- [ ] **K4** Address findings (max 3 push cycles).
- [ ] **K5** Squash merge.
- [ ] **K6** Close issue #258.
- [ ] **K7** Tag `v2026.5.30NN` + push.
- [ ] **K8** Verify Release workflow → npm publish both packages → confirm new version visible.
- [ ] **K9** `openspec archive restore-summary-disk-persistence --yes` + commit.
- [ ] **K10** Worktree cleanup.
